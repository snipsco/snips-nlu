...
from snips_nlu.nlu_engine.nlu_engine import IntentParser
from snips_nlu.result import (intent_classification_result,
                                         parsed_entity, result)
from snips_nlu.utils import (LimitedSizeDict, transform_to_rasa_format)
...
class RasaIntentParser(IntentParser):
    ...
    def fit(self, dataset):
        ...
        self.trainer.persist(dir_name, persistor=None, create_unique_subfolder=False)

        metadata = json.loads(open(os.path.join(dir_name, 'metadata.json')).read())
        
        if self.backend == 'spacy_sklearn':
            from rasa_nlu.interpreters.spacy_sklearn_interpreter import (SpacySklearnInterpreter)
            self.interpreter = SpacySklearnInterpreter(**metadata)
