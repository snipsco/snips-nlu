...
def augment_dataset(dataset, language='en'):
	...
	intent_list = dataset.queries.keys()
    ...
    for intent in intent_list:
    	queries += [ ''.join( [ dataset.queries[intent][i]['data'][j]['text'] for j in range(len(dataset.queries[intent][i]['data'])) ] ) for i in range(len(dataset.queries[intent])) ]
    	y +=  [ intent_code[intent] for _ in xrange(len(dataset.queries[intent])) ]
