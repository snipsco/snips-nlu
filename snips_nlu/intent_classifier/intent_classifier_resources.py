...
def get_stop_words(language):
    ...
    if STOP_WORDS is None:
        stop_words_file_path = os.path.join(get_resources_path(language), 'stop_words.txt')
...
def get_subtitles(language):
    ...
    if SUBTITLES is None:
        subtitles_file_path = os.path.join(get_resources_path(language), 'subtitles.txt')
